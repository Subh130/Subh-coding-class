...
strong_enemy = {
    ...
    'x_change': 2 * enemy_speed_multiplier,
    'y_change': 40,
    'health': 2
}
...
level_font = pygame.font.Font("freesansbold.ttf", 28)
...
# [LEVEL] Show Level
def show_level(x, y):
    level = level_font.render("Level: " + str(current_level), True, (255, 255, 255))
...
while running:
    ...
    if not game_over:
        ...
        for i in range(num_of_enemies):
            ...
            if enemyX[i] <= 0:
                enemyX_change[i] = 4 * enemy_speed_multiplier
        ...
        if strong_enemy['x'] <= 0:
            strong_enemy['x_change'] = 2 * enemy_speed_multiplier
        ...
        # [LEVEL] Level up logic
        if score_value >= current_level * level_up_score:
            current_level += 1
            enemy_speed_multiplier += 0.2
            # Speed up enemies
            for i in range(num_of_enemies):
                # Scale enemy speed with multiplier and direction
                enemyX_change[i] = (4 if enemyX_change[i] > 0 else -4) * enemy_speed_multiplier
            # Speed up strong enemy
            strong_enemy['x_change'] = (2 if strong_enemy['x_change'] > 0 else -2) * enemy_speed_multiplier

            show_level_up_message = True
            level_up_ticks = pygame.time.get_ticks()
    ...
    if show_level_up_message:
        if pygame.time.get_ticks() - level_up_ticks < 2000:
            level_up_text = level_font.render("LEVEL UP!", True, (255, 255, 0))
            screen.blit(level_up_text, (330, 100))
        else:
            show_level_up_message = False
